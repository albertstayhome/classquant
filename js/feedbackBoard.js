...
class FeedbackBoard {
    ...
    container.innerHTML = `
      ...
      <div class="glass-card rounded-3xl p-5 sm:p-6 mb-5 border border-pink-200 bg-white shadow-sm">
        <div class="flex flex-wrap items-center justify-between gap-4">
          <div class="flex items-center space-x-3.5">
            ...
            <div>
              <div class="flex flex-wrap items-center gap-2">
                ...
                <span class="px-2.5 py-0.5 rounded-full bg-slate-100 text-slate-700 text-xs font-bold border border-slate-300 shadow-sm flex items-center gap-1">
                  <span>共 ${totalCount} 則</span>
                  <span>•</span>
                  <span>已回覆 ${repliedCount} 則</span>
                </span>
                ${isDev ? `
                  <span class="px-2.5 py-0.5 rounded-full bg-purple-100 text-purple-800 text-xs font-bold border border-purple-300 shadow-sm flex items-center gap-1">
                    <span>開發者模式</span>
                  </span>
                ` : ''}
              </div>
              <p class="text-xs sm:text-sm text-slate-600 font-medium mt-1">
                提供使用者回饋意見、提出功能需求或回報問題。開發者可在此檢視並回覆。
              </p>
            </div>
          </div>
          ...
          <!-- Action Buttons -->
          <div class="flex flex-wrap items-center gap-2">
            <button onclick="feedbackBoard.openPostModal()" class="px-4 py-2.5 rounded-2xl bg-pink-600 hover:bg-pink-700 text-white text-xs sm:text-sm font-bold flex items-center gap-1.5 transition shadow-sm active:scale-95">
              <i data-lucide="edit-3" class="w-4 h-4"></i>
              <span>✍️ 新增留言</span>
            </button>
            <button onclick="feedbackBoard.refresh()" class="px-3 py-2 rounded-xl bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 text-xs font-bold flex items-center gap-1 transition active:scale-95 shadow-sm" title="重新整理">
              <i data-lucide="rotate-cw" class="w-3.5 h-3.5"></i>
              <span class="hidden sm:inline">重新整理</span>
            </button>
            <button onclick="feedbackBoard.copyAllForDev()" class="px-3 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-300 text-xs font-bold flex items-center gap-1 transition active:scale-95 shadow-sm" title="複製留言清單">
              <i data-lucide="copy" class="w-3.5 h-3.5"></i>
              <span class="hidden sm:inline">複製清單</span>
            </button>
            <button onclick="feedbackBoard.openSyncSettingsModal()" class="px-2.5 py-2 rounded-xl bg-slate-50 hover:bg-slate-100 text-slate-600 border border-slate-200 text-xs font-bold flex items-center gap-1 transition active:scale-95 shadow-sm" title="設定">
      ...
      <div class="space-y-3.5">
        ${items.length === 0 ? `
          <div class="glass-card rounded-3xl p-8 text-center border border-pink-200 bg-white shadow-sm">
            <div class="w-12 h-12 mx-auto mb-2 bg-slate-100 rounded-full flex items-center justify-center text-xl text-slate-500">
              💬
            </div>
            <h3 class="text-sm font-bold text-slate-700 mb-1">目前尚無留言</h3>
            <p class="text-xs text-slate-500 mb-3 font-medium">如有任何需求或問題，歡迎點擊下方按鈕新增留言。</p>
            <button onclick="feedbackBoard.openPostModal()" class="px-3.5 py-2 rounded-xl bg-pink-600 text-white text-xs font-bold hover:bg-pink-700 transition shadow-sm">
              新增留言
            </button>
          </div>
  ...
  renderFeedbackCard(item, isDev) {
    ...
    return `
      <div class="glass-card rounded-2xl p-4 sm:p-5 border border-slate-200 bg-white shadow-sm hover:shadow transition">
        <!-- Card Header -->
        <div class="flex items-start justify-between gap-3 mb-2.5">
          <div>
            <div class="flex flex-wrap items-center gap-2">
              <span class="font-bold text-slate-900 text-sm">${this.escapeHtml(item.author || '訪客')}</span>
              <span class="px-2 py-0.5 rounded text-[11px] font-medium ${categoryConfig.bg} border">
          ...
          <div class="flex items-center space-x-1.5 shrink-0">
            <button onclick="feedbackBoard.toggleLike('${item.id}')" class="px-2 py-1 rounded-lg bg-slate-50 hover:bg-slate-100 text-slate-600 border border-slate-200 text-xs font-medium transition flex items-center gap-1 active:scale-95 shadow-sm" title="表示贊同">
              <span>👍</span>
              <span id="like-count-${item.id}" class="font-bold">${item.likes || 0}</span>
            </button>
            ${isDev ? `
              <button onclick="feedbackBoard.openReplyModal('${item.id}')" class="px-2.5 py-1 rounded-lg bg-purple-50 hover:bg-purple-100 text-purple-700 border border-purple-200 text-xs font-bold transition flex items-center gap-1 active:scale-95 shadow-sm">
        ...
        <!-- Feedback Content -->
        <div class="text-xs sm:text-sm text-slate-800 leading-relaxed font-normal pl-0.5 mb-2.5 whitespace-pre-wrap">
          ${this.escapeHtml(item.content)}
        </div>

        <!-- Developer Reply Card (if any) -->
        ${hasReply ? `
          <div class="mt-2.5 p-3 rounded-xl border border-slate-200 bg-slate-50 text-slate-800 relative">
        ...
        ` : (isDev ? `
          <div class="mt-1 text-right">
            <button onclick="feedbackBoard.openReplyModal('${item.id}')" class="text-xs text-purple-600 hover:text-purple-800 underline transition inline-flex items-center gap-1">
              <span>回覆此則留言</span>
            </button>
          </div>
        ` : '')}
    ...
    modalContent.innerHTML = `
      <div class="p-5 sm:p-6 animate-fade-in-up">
        ...
        <div class="flex items-center justify-between pb-3 mb-4 border-b border-slate-200">
          <div>
            <h3 class="font-bold text-slate-900 text-base">新增留言</h3>
            <p class="text-xs text-slate-500">提出功能建議、回報問題或使用意見。</p>
          </div>
        ...
        <form id="feedback-post-form" onsubmit="event.preventDefault(); feedbackBoard.submitFeedback();" class="space-y-3.5">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label class="block text-xs font-bold text-slate-700 mb-1">暱稱 / 稱謂 *</label>
              <input type="text" id="fb-author-input" value="${this.escapeHtml(savedAuthor)}" placeholder="例如：數學老師、801導師" class="w-full bg-slate-50 border border-slate-300 rounded-xl px-3 py-2 text-xs text-slate-900 focus:outline-none focus:border-slate-500" required>
            </div>
            <div>
              <label class="block text-xs font-bold text-slate-700 mb-1">角色身份</label>
              <select id="fb-role-select" class="w-full bg-slate-50 border border-slate-300 rounded-xl px-3 py-2 text-xs text-slate-900 focus:outline-none focus:border-slate-500">
          ...
          <!-- Row 2: Category -->
          <div>
            <label class="block text-xs font-bold text-slate-700 mb-1">類別</label>
            <select id="fb-category-select" class="w-full bg-slate-50 border border-slate-300 rounded-xl px-3 py-2 text-xs text-slate-900 focus:outline-none focus:border-slate-500">
          ...
          <!-- Row 3: Content -->
          <div>
            <label class="block text-xs font-bold text-slate-700 mb-1">留言內容 *</label>
            <textarea id="fb-content-input" rows="4" placeholder="請輸入您的建議、問題或回饋內容..." class="w-full bg-slate-50 border border-slate-300 rounded-xl p-3 text-xs text-slate-900 focus:outline-none focus:border-slate-500 leading-relaxed" required></textarea>
          </div>

          <!-- Submit Buttons -->
          <div class="flex items-center justify-end space-x-2 pt-2 border-t border-slate-100">
            <button type="button" onclick="appState.closeModal()" class="px-3.5 py-2 rounded-xl text-slate-600 hover:bg-slate-100 text-xs font-medium transition">
              取消
            </button>
            <button type="submit" id="fb-submit-btn" class="px-4 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold transition flex items-center gap-1.5 shadow-sm active:scale-95">
  ...
  openReplyModal(feedbackId) {
    ...
    modalContent.innerHTML = `
      <div class="p-5 sm:p-6 animate-fade-in-up">
        ...
        <div class="flex items-center justify-between pb-3 mb-4 border-b border-slate-200">
          <div>
            <h3 class="font-bold text-slate-900 text-base">回覆留言</h3>
            <p class="text-xs text-slate-500">回覆將直接顯示於該留言下方。</p>
          </div>
          <button onclick="appState.closeModal()" class="w-7 h-7 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-500 flex items-center justify-center transition">
            ✕
          </button>
        ...
        <div class="p-3 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-700 mb-3.5">
          ...
          <div class="font-normal whitespace-pre-wrap pl-1 border-l-2 border-slate-300">${this.escapeHtml(item.content)}</div>
        </div>
        ...
        <div class="space-y-3">
          <div>
            ...
            <textarea id="dev-reply-input" rows="4" placeholder="請輸入回覆內容..." class="w-full bg-slate-50 border border-slate-300 rounded-xl p-3 text-xs text-slate-900 focus:outline-none focus:border-slate-500 leading-relaxed">${this.escapeHtml(item.reply || '')}</textarea>
          </div>

          <div class="flex items-center justify-between pt-2 border-t border-slate-100">
            ${item.reply ? `
              <button type="button" onclick="feedbackBoard.deleteReply('${item.id}')" class="px-3 py-1.5 rounded-xl text-rose-600 hover:bg-rose-50 text-xs font-medium transition">
                清空回覆
              </button>
            ` : '<div></div>'}
            <div class="flex items-center space-x-2">
              ...
              <button type="button" onclick="feedbackBoard.saveReply('${item.id}')" class="px-4 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold transition flex items-center gap-1.5 shadow-sm active:scale-95">
